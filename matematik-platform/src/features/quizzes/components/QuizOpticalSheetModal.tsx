...
export const QuizOpticalSheetModal: React.FC<QuizOpticalSheetModalProps> = ({
  ...
  const content = (
    ...
    >
      ...
      <div className="border-b border-amber-200 dark:border-slate-800 bg-amber-100/70 dark:bg-slate-950/80 px-4 sm:px-6 py-4">
          ...
          <div className="flex items-center gap-1">
            {onToggleDock && (
              <button
                ...
                title={isDocked ? 'Görünümü pop-up modala al' : 'Ekranın sağına sabitle'}
      ...
      <div className="overflow-y-auto p-4 sm:p-5 flex-1 bg-amber-50/40 dark:bg-slate-900/60">
        <div className={isDocked ? 'space-y-2.5' : 'grid grid-cols-1 sm:grid-cols-2 gap-3'}>
          {Array.from({ length: totalQuestions }).map((_, qIdx) => {
            ...
            return (
              ...
              >
                ...
                {/* Bubble Options (A, B, C, D, [E]) */}
                <div className="flex items-center gap-1.5">
                  {activeOptionLetters.map((letter, optIdx) => {
  ...
  return (
    <div className="fixed inset-0 z-[140] flex items-center justify-center p-4">
      ...
      <div
        ...
        aria-hidden="true"
      />
      <div className="relative z-10 w-full max-w-xl animate-in fade-in zoom-in-95 duration-200">
        {content}
      </div>
    </div>
  );
