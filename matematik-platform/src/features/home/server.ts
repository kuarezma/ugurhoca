...
export const loadHomeFeedServer = async (): Promise<HomeFeedPayload> => {
  const emptyPayload: HomeFeedPayload = {
    announcements: [],
    documents: [],
    writings: [],
  };
  ...
  try {
    ...
    // Env variables may be absent during CI builds; fall back to empty data
    // so the client-side hook fetches at runtime instead.
    return emptyPayload;
  }
};
