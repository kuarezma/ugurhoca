    ...
    <main className="page-surface min-h-screen gradient-bg px-4 pb-16 pt-20 sm:px-6 sm:pt-24">
      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-6 flex items-center gap-2 text-xs font-semibold text-secondary">
          ...
          <span>/</span>
          <span className="text-emerald-800 dark:text-emerald-300 font-bold">EBOB - EKOK Hesaplayıcı</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          <div className="lg:col-span-5 space-y-4">
            <div className="rounded-3xl border border-default bg-surface-1 p-5 shadow-xl backdrop-blur-md space-y-4">
              ...
              <div>
                <label htmlFor="num1-input" className="text-xs font-semibold text-secondary block mb-1.5">
                  1. Sayı (a)
                </label>
                <input
                  id="num1-input"
                  ...
                  className="w-full h-11 rounded-xl border border-default bg-surface-2 px-4 font-mono text-base font-bold text-primary focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>

              <div>
                <label htmlFor="num2-input" className="text-xs font-semibold text-secondary block mb-1.5">
                  2. Sayı (b)
                </label>
                <input
                  id="num2-input"
            ...
            <div className="rounded-3xl border border-default bg-surface-1 p-5 shadow-2xl backdrop-blur-md space-y-3">
              ...
              <div className="flex items-center justify-between border-t border-default pt-2">
                ...
                <span className="text-2xl font-black font-mono text-teal-600 dark:text-teal-300">{ekok}</span>
              </div>

              {areCoprime && (
                <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-2.5 text-xs text-amber-700 dark:text-amber-300 font-semibold flex items-center gap-1.5 mt-2">
              ...
              <div className="border-t border-default pt-2 text-[11px] font-mono text-secondary">
            ...
            <div className="rounded-3xl border border-default bg-surface-1 p-5 shadow-xl backdrop-blur-md space-y-4">
              <div className="flex items-center justify-between border-b border-default pb-3">
                <h3 className="text-xs font-bold uppercase tracking-wider text-secondary flex items-center gap-1.5">
                  <Sparkles className="h-3.5 w-3.5 text-emerald-800 dark:text-emerald-300" />
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-center font-mono text-sm">
                  <thead>
                    <tr className="border-b border-default text-xs text-secondary">
                  ...
                  <tbody>
                    {steps.map((step, idx) => (
                      <tr
                        key={idx}
                        className={`border-b border-subtle transition ${
                          step.dividesAll ? 'bg-emerald-500/10 font-bold' : ''
                        }`}
                      >
                        <td className="py-2 px-3 text-primary">{step.nums[0]}</td>
                        <td className="py-2 px-3 text-primary">{step.nums[1]}</td>
                        <td className="py-2 px-3 text-right flex items-center justify-end gap-1.5 font-bold">
                          <span className={step.dividesAll ? 'text-emerald-800 dark:text-emerald-300' : 'text-secondary'}>
                            {step.prime}
                          </span>
                          {step.dividesAll ? (
                            <span className="text-xs text-amber-700 dark:text-amber-300" title="Her iki sayıyı böldü">
                              ★
                            </span>
                          ) : (
              ...
              <div className="rounded-2xl border border-default bg-surface-2 p-3.5 text-xs text-secondary space-y-1.5">
                <div>
                  <span className="font-bold text-emerald-800 dark:text-emerald-300">EBOB Hesabı: </span>
                ...
                <div>
                  <span className="font-bold text-teal-800 dark:text-teal-300">EKOK Hesabı: </span>
