    ...
    <div className="space-y-6">
      {topicGroups.map((group, groupIdx) => {
        ...
        return (
          <div
            key={group.topicName}
            className="overflow-hidden rounded-3xl border border-slate-200 dark:border-white/10 bg-white/95 dark:bg-slate-900/60 shadow-xl backdrop-blur-xl transition-all"
          >
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white group-hover:text-purple-600 dark:group-hover:text-purple-300">
                      {group.topicName}
                    </h3>
                    <span className="rounded-full border border-purple-400/30 bg-purple-50 text-purple-700 dark:bg-purple-500/15 dark:text-purple-200 px-2.5 py-0.5 text-xs font-semibold">
                      {totalDocs} Doküman
                    </span>
                  </div>
              ...
              <div className="flex items-center justify-between sm:justify-end gap-5">
                ...
                <div className="rounded-xl border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-slate-800/80 p-2 text-slate-600 dark:text-slate-300 transition-transform">
                  {isExpanded ? (
            ...
            <AnimatePresence>
              {isExpanded && (
                <motion.div
                  ...
                  className="border-t border-slate-200 dark:border-white/10 bg-slate-50/50 dark:bg-slate-950/30 p-4 sm:p-6"
                >
