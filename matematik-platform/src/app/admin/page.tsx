...
import {
  ...
  BarChart3, Clock, ChevronRight, Star, BellRing, BellOff, CheckCheck, Reply
} from 'lucide-react';
          ...
          <div className="p-5 border-b border-white/5 bg-gradient-to-r from-indigo-500/10 to-purple-500/10 flex items-center justify-between shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-indigo-200/60 text-xs font-medium">Öğrenci mesajları ve istekler</p>
              </div>
            </div>
          ...
          <div className="overflow-y-auto flex-1 custom-scrollbar pb-2">
           {notifications.length === 0 ? (
             <div className="flex flex-col items-center justify-center p-12 text-center opacity-70">
               ...
               <p className="text-slate-500 text-sm mt-1">Sistemde mesaj veya bildirim bulunmuyor.</p>
             </div>
           ) : (
             <div className="flex flex-col gap-1.5 p-2">
               {notifications.map((notif) => {
                 ...
                 const isCevap = notif.title.toLowerCase().includes('cevapladı');
                 const isMesaj = notif.title.toLowerCase().includes('sana yazdı');
                 ...
                 return (
                  ...
                  >
                    {isNew && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-indigo-500 rounded-r-full shadow-[0_0_10px_rgba(99,102,241,0.5)]" />}
                    
                    <div className="flex gap-4">
                      {/* Icon Container */}
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 border shadow-inner ${
                        !isNew 
                         ? 'bg-slate-800 border-slate-700/50 text-slate-500' 
                         : isCevap ? 'bg-emerald-500/20 border-emerald-500/30 text-emerald-400 shadow-emerald-500/10'
                         : isOkundu ? 'bg-blue-500/20 border-blue-500/30 text-blue-400 shadow-blue-500/10'
                         : isTeslim ? 'bg-cyan-500/20 border-cyan-500/30 text-cyan-400 shadow-cyan-500/10'
                         : isMesaj ? 'bg-purple-500/20 border-purple-500/30 text-purple-400 shadow-purple-500/10'
                         : 'bg-indigo-500/20 border-indigo-500/30 text-indigo-400 shadow-indigo-500/10'
                      }`}>
                        {isOkundu ? <CheckCheck className="w-6 h-6" /> 
                         : isTeslim ? <Check className="w-6 h-6" /> 
                         : isCevap ? <Reply className="w-6 h-6" />
                         : isMesaj ? <MessageSquareText className="w-6 h-6" />
                         : <Bell className="w-6 h-6" />}
                      </div>
                      
                      <div className="flex-1 min-w-0 flex flex-col justify-center">
                        <div className="flex items-start justify-between gap-3 mb-1">
                          <p className={`font-semibold text-sm truncate ${!isNew ? 'text-slate-400' : 'text-white'}`}>
                            {notif.title}
                          </p>
                          {isNew && (
                        ...
                        <p className={`text-xs line-clamp-2 leading-relaxed ${!isNew ? 'text-slate-500' : 'text-slate-300'}`}>
