...
export default function AdminPage() {
  ...
  const loadData = async () => {
    ...
    // Bildirimleri getir — SADECE adminin kendi gelen kutusundaki mesajlar
    const {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (modalType === "assignment") {
      ...
      if (!error) {
        setQuizzes(
          quizzes.map((q) =>
            q.id === selectedQuiz.id ? { ...q, ...formData } : q,
          ),
        );
      }
  ...
  const editAssignment = async (assignment: any) => {
    ...
    if (!error) {
      setAssignments(
        assignments.map((a) =>
          a.id === assignment.id ? { ...a, title, description } : a,
        ),
      );
    }
  };
  ...
  const editSharedDocument = async (doc: any) => {
    ...
    if (!error) {
      setSharedDocs(
        sharedDocs.map((d) =>
          d.id === doc.id ? { ...d, document_title, file_url } : d,
        ),
      );
    }
  };
  ...
  const adminTargetIds = new Set(
    [user?.id, ...adminProfileIds].filter(Boolean),
  );
  ...
  const unreadNotifications = notifications.filter(
    (n) => isIncomingAdminMessage(n) && !n.is_read,
  );
  ...
  const getSenderActionStatus = (senderId?: string) => {
    ...
    for (const n of notifications) {
      ...
      if (mod.action === "block")
        return { blocked: true, muted: false, expires_at: null };

      if (mod.action === "mute") {
        const active =
          mod.expires_at && new Date(mod.expires_at).getTime() > Date.now();
        if (active)
          return { blocked: false, muted: true, expires_at: mod.expires_at };
      }
    }
  ...
  const studentMessages = notifications.filter((n) =>
    isIncomingAdminMessage(n),
  );

  const groupedMessages = studentMessages.reduce(
    (acc, msg) => {
      ...
      {showNotifications && (
        ...
        >
          ...
          <div className="overflow-y-auto flex-1 custom-scrollbar pb-2">
            {notifications.filter(isIncomingAdminMessage).length === 0 ? (
              ...
              <div className="flex flex-col gap-1.5 p-2">
                {notifications.filter(isIncomingAdminMessage).map((notif) => {
                  const isOkundu = notif.title.toLowerCase().includes("okundu");
                  const isTeslim = notif.title.toLowerCase().includes("teslim");
                  const isCevap = notif.title
                    .toLowerCase()
                    .includes("cevapladı");
                  const isMesaj = notif.title
                    .toLowerCase()
                    .includes("sana yazdı");
                      ...
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-center">
                          <div className="flex items-start justify-between gap-3 mb-1">
                            <p
                              className={`font-semibold text-sm truncate ${!isNew ? "text-slate-400" : "text-white"}`}
                            >
                              {notif.title}
                            </p>
                            {isNew && (
                          ...
                          <p
                            className={`text-xs line-clamp-2 leading-relaxed ${!isNew ? "text-slate-500" : "text-slate-300"}`}
                          >
          ...
          >
            ...
            >
              ...
              <div className="p-5 space-y-4">
                ...
                {parseMessagePayload(selectedNotification)?.metadata && (
                  <div className="rounded-xl bg-slate-800/60 border border-slate-700 p-3 text-xs text-slate-300 space-y-1">
                    ...
                    <p>
                      Zaman:{" "}
                      {new Date(
                        parseMessagePayload(selectedNotification)?.created_at ||
                          selectedNotification.created_at,
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {getSenderActionStatus(
                      ...
                      <span className="px-2 py-1 rounded-full text-[11px] bg-amber-500/20 text-amber-300">
                        ...
                        {new Date(
                          getSenderActionStatus(
                            parseMessagePayload(selectedNotification)
                              ?.sender_id,
                          ).expires_at,
                        ).toLocaleDateString("tr-TR")}
                        )
                ...
                {parseMessagePayload(selectedNotification)?.attachments
                  ?.length > 0 && (
                ...
                {announcements.length === 0 ? (
                  <div className="glass rounded-3xl p-12 sm:p-16 text-center border border-white/5">
                    ...
                    <p className="text-slate-400 max-w-md mx-auto">
                      Sistemde yayınlanan hiçbir bildirim/duyuru bulunmuyor.
                      Yeni bir duyuru ekleyerek öğrencilerinizi
                      ...
                      >
                        ...
                        <div className="p-6 flex-1 flex flex-col">
                          ...
                          <div className="flex items-center gap-2 pt-4 border-t border-white/5 mt-auto">
                            ...
                            <button
                              onClick={() =>
                                deleteItem("announcement", announcement.id)
                              }
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex items-center gap-4 flex-1">
                            <div
                              className={`w-14 h-14 rounded-xl bg-gradient-to-br ${typeColors[doc.type] || "from-slate-500 to-slate-600"} flex items-center justify-center flex-shrink-0`}
                            >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-3 mb-1">
                                ...
                                <span
                                  className={`px-3 py-1 bg-gradient-to-r ${typeColors[doc.type] || "from-slate-500 to-slate-600"} rounded-full text-white text-xs font-semibold flex-shrink-0`}
                                >
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-slate-500 text-xs mt-1">
                                  Sınıf: {u.grade || "Belirtilmemiş"} • Kayıt:{" "}
                                  {u.created_at
                            ...
                            <div className="flex items-center gap-2 flex-wrap justify-end">
                              ...
                              >
                                <Star
                                  className={`w-4 h-4 ${u.is_private_student ? "fill-amber-500" : ""}`}
                                />
                ...
                {privateStudents.length === 0 ? (
                  <div className="glass rounded-2xl p-8 sm:p-12 text-center">
                    ...
                    <p className="text-slate-500 text-sm">
                      Öğrenci eklemek için profil sayfasından "Özel Ders
                      Öğrencisi" seçeneğini kullanın
                    </p>
                  </div>
                ) : (
                            ...
                            <div className="space-y-2">
                              {studentAssignments.slice(0, 3).map((assign) => (
                                ...
                                >
                                  ...
                                  <button
                                    onClick={() =>
                                      deleteItem("assignment", assign.id)
                                    }
                      ...
                      {chatRooms.map((room) => {
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-10 h-10 rounded-lg flex items-center justify-center ${isActive ? "bg-indigo-500 text-white" : "bg-slate-800 text-slate-400"}`}
                              >
                              ...
                              return (
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    <p
                                      className={`text-[9px] mt-1 text-right ${isAdmin ? "text-white/60" : "text-slate-500"}`}
                                    >
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="text-sm">
                        ...
                        <p className="text-slate-400">
                          ...
                          Örneğin: 5. sınıf → 6. sınıf, 12. sınıf → 12. sınıf
                          (sabit kalır)
                        </p>
                      </div>
                    </div>
                  ...
                  <motion.button
                    ...
                    onClick={async () => {
                      ...
                      for (const u of allUsers) {
                        if (u.isAdmin || u.grade >= 12 || u.grade === "Mezun")
                          continue;
                        await supabase
                          .from("profiles")
                          .update({ grade: u.grade + 1 })
                          .eq("id", u.id);
                      }
                    ...
                    {sharedDocs.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {sharedDocs.map((doc) => (
                          <div
                            key={doc.id}
                            className="bg-slate-800/50 rounded-lg p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3"
                          >
                            ...
                            <div className="flex items-center gap-2 self-end sm:self-auto">
                              <span
                                className={`px-2 py-1 rounded-full text-xs ${doc.is_read ? "bg-green-500/20 text-green-400" : "bg-yellow-500/20 text-yellow-400"}`}
                              >
                                {doc.is_read ? "Görüldü" : "Bekliyor"}
                              </span>
                              <button
                                onClick={() => editSharedDocument(doc)}
                              ...
                              <button
                                onClick={() =>
                                  deleteItem("shared_document", doc.id)
                                }
              ...
              {success ? (
                ...
                <form
                  onSubmit={async (e) => {
                    ...
                    const selectedStudentUser = allUsers.find(
                      (u) => u.id === formData.student_id,
                    );
                      ...
                      .insert([
                        {
                          ...
                          document_title:
                            selectedDoc?.title || formData.document_title,
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          answer_key_text: e.target.value,
                        })
                      }
                      rows={3}
                      className="w-full bg-slate-800/50 border border-slate-700 rounded-xl px-4 py-3 text-white
                  ...
                  onSubmit={async (e) => {
                    ...
                    const { data, error } = await supabase
                      ...
                      .insert([
                        {
                          name: formData.name,
                          name_normalized: normalizeFullNameForMatch(
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  {(modalType === "announcement" ||
                    ...
                    <div>
                      ...
                      />
                      <p className="text-xs text-slate-500 mt-2">
                        PDF, site veya başka bir detay bağlantısı
                        ekleyebilirsin.
                      </p>
                    </div>
                  )}
                    ...
                    <textarea
                      ...
                      placeholder={
                        modalType === "document"
                          ...
                          : modalType === "assignment"
                            ? "Hangi sayfalar / kaynaklar yapılacak?"
                            : modalType === "quiz" || modalType === "editQuiz"
                              ? "Test hakkında bilgi..."
                              : "İçeriği buraya yazın..."
                      }
                    />
                  </div>
                    ...
                    <>
                      ...
                      <div>
                        <label className="block text-amber-500 mb-2 font-bold uppercase tracking-wider text-xs flex items-center gap-1 mt-4">
                          <AlertCircle className="w-3.5 h-3.5" /> Çözüm /
                          Açıklama (Opsiyonel)
                        </label>
                        <textarea
                          rows={2}
                            ...
                            onChange={async (e) => {
                              ...
                              if (file) {
                                ...
                                try {
                                  ...
                                  alert(
                                    "Excel dosyası okunamadı: " + error.message,
                                  );
                                }
                              }
                            }}
                        ...
                        {formData.importResult && (
                          <div className="space-y-3">
                            ...
                            {formData.importResult.errors.length > 0 && (
                              <div className="p-3 bg-red-500/10 border border-red-500/30 rounded-lg">
                                <p className="text-red-400 text-sm font-medium mb-2">
                                  {formData.importResult.errors.length} hata
                                  bulundu:
                                </p>
                                <ul className="text-red-300 text-xs space-y-1">
                                  {formData.importResult.errors.map(
                                    (err: any, idx: number) => (
                                      <li key={idx}>
                            ...
                            <div className="max-h-40 overflow-y-auto space-y-1">
                              {formData.importResult.valid
                                .slice(0, 10)
                                .map((q: any, idx: number) => (
                                  <div
                                    key={idx}
                                    className="p-2 bg-slate-800/30 rounded text-xs"
                                  >
                              ...
                              {formData.importResult.valid.length > 10 && (
                                <p className="text-slate-500 text-xs text-center">
                                  ... ve{" "}
                                  {formData.importResult.valid.length - 10} soru
                                  daha
                                </p>
                              )}
                            </div>
                      ...
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              is_admin_only: e.target.checked,
                            })
                          }
                        ...
                        <div className="flex flex-wrap gap-2">
                          {[5, 6, 7, 8, 9, 10, 11, 12].map((grade) => (
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={
                                  formData.grades?.includes(grade) || false
                                }
                                onChange={(e) => {
                          ...
                          <label className="flex items-center gap-2 px-3 py-2 glass rounded-lg cursor-pointer hover:bg-white/10 min-w-[calc(50%-0.25rem)] sm:min-w-0">
                            <input
                              type="checkbox"
                              checked={
                                formData.grades?.includes("Mezun") || false
                              }
                              onChange={(e) => {
                                const grades = formData.grades || [];
                                if (e.target.checked) {
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 pt-2 items-start sm:items-center">
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto mt-2 sm:mt-0">
                          ...
                          <div className="flex items-center justify-center p-2 bg-slate-800 border border-slate-700 rounded-xl">
                            {[1, 2, 3, 4, 5].map((star) => {
                              const currentGradeObj = document.getElementById(
                                `grade-${sub.id}`,
                              ) as HTMLInputElement;
                              const currentGrade = currentGradeObj
                                ? parseInt(currentGradeObj.value) || 0
                                : sub.grade || 0;
                              ...
                              return (
                                <button
                                  ...
                                  onClick={(e) => {
                                    ...
                                    if (gradeInput)
                                      gradeInput.value = (star * 20).toString();
