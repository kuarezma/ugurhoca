...
export function ExamCountdown({ exam, isLight }: ExamCountdownProps) {
  ...
  return (
    ...
    >
      <motion.div
        animate={{ scale: [1, 1.08, 1], opacity: isLight ? [0.18, 0.26, 0.18] : [0.26, 0.36, 0.26] }}
        transition={{ duration: 4.8, repeat: Infinity, ease: 'easeInOut' }}
        className={`absolute -top-10 -right-8 h-24 w-24 rounded-full bg-gradient-to-br ${exam.accent} blur-3xl`}
      />
      <motion.div
        animate={{ x: [0, 4, 0], y: [0, -4, 0], opacity: isLight ? [0.14, 0.22, 0.14] : [0.22, 0.3, 0.22] }}
        transition={{ duration: 5.6, repeat: Infinity, ease: 'easeInOut' }}
        className={`absolute -bottom-10 -left-8 h-20 w-20 rounded-full bg-gradient-to-br ${exam.accent} blur-3xl`}
      />
      {!isLight && <div className={`absolute inset-0 bg-gradient-to-br ${exam.accent} opacity-[0.1]`} />}
      <div className={`absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r ${exam.accent}`} />
      {[0, 1, 2].map((index) => (
        <motion.span
          key={index}
          aria-hidden="true"
          animate={{ y: [0, -5, 0], scale: [1, 1.15, 1], opacity: [0.35, 0.8, 0.35] }}
          transition={{ duration: 2.6 + index * 0.35, repeat: Infinity, ease: 'easeInOut', delay: index * 0.25 }}
      ...
      <div className="relative flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="mb-1.5 flex flex-wrap items-center gap-1.5">
            <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-bold uppercase tracking-[0.16em] ${isLight ? 'border border-white/70 bg-gradient-to-r from-white to-slate-50 text-slate-700 shadow-sm' : 'bg-white/10 text-white border border-white/15 shadow-[0_6px_16px_rgba(15,23,42,0.28)]'}`}>
              <Clock3 className="w-3.5 h-3.5" />
              {exam.provider}
            </span>
            <span className={`inline-flex items-center rounded-full bg-gradient-to-r ${exam.accent} px-2 py-1 text-[10px] font-bold text-white shadow-sm`}>
              {status}
            </span>
          </div>
          <h3 className={`truncate text-sm sm:text-[15px] font-black ${isLight ? 'light-text-strong' : 'text-white'}`}>{exam.title}</h3>
        </div>
      ...
      <div className="relative mt-3 grid grid-cols-3 gap-2">
        {[
          ...
          >
            <div className={`text-lg sm:text-xl font-black leading-none ${isLight ? 'bg-gradient-to-br from-slate-950 via-indigo-700 to-fuchsia-600 bg-clip-text text-transparent' : 'bg-gradient-to-br from-white via-cyan-200 to-fuchsia-300 bg-clip-text text-transparent'}`}>{item.value}</div>
            <div className={`mt-1.5 text-[9px] font-semibold uppercase tracking-[0.2em] ${isLight ? 'text-slate-500' : 'text-slate-300'}`}>{item.label}</div>
          </motion.div>
        ))}
      </div>

      <div className={['relative mt-2.5 rounded-2xl border p-2.5', isLight ? 'border-indigo-100 bg-gradient-to-r from-indigo-50 via-white to-pink-50/80 shadow-sm' : 'bg-gradient-to-r from-white/10 via-white/5 to-transparent border-white/10 backdrop-blur-sm'].join(' ')}>
        <div className={`text-[10px] font-bold uppercase tracking-[0.16em] ${isLight ? 'text-slate-700' : 'text-white'}`}>Sinav Takvimi</div>
        <div className={`mt-1 text-[11px] sm:text-xs font-medium ${isLight ? 'text-slate-600' : 'text-slate-300'}`}>{exam.dateLabel}</div>
